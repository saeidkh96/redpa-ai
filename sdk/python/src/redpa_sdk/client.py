...
class RedPA:
    ...
    def analytics_catalog(self) -> dict[str, Any]:
        return self._request("GET", "/api/v1/analytics/catalog")

    def ingest_analytics(self, items: list[dict[str, Any]]) -> dict[str, Any]:
        return self._request("POST", "/api/v1/analytics/events", json={"items": items})

    def query_kpi(self, payload: dict[str, Any]) -> dict[str, Any]:
        return self._request("POST", "/api/v1/analytics/query", json=payload)

    def connectors(self, *, limit: int = 100) -> list[dict[str, Any]]:
        return self._request("GET", "/api/v1/connectors", params={"limit": limit})

    def create_connector(self, payload: dict[str, Any]) -> dict[str, Any]:
        return self._request("POST", "/api/v1/connectors", json=payload)

    def execute_connector(self, connector_id: str, *, payload: dict[str, Any], approval_granted: bool = False, dry_run: bool = True) -> dict[str, Any]:
        return self._request("POST", f"/api/v1/connectors/{connector_id}/execute", json={"payload": payload, "approval_granted": approval_granted, "dry_run": dry_run})
