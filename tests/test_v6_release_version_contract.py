...
def test_current_release_version_contract():
    ...
    assert 'default="9.0.0"' in config_source
    assert 'APP_VERSION: "9.0.0"' in compose
    assert frontend["version"] == "9.0.0"
    assert 'version = "9.0.0"' in sdk
    assert 'appVersion: "9.0.0"' in helm
    assert 'APP_VERSION: "9.0.0"' in ci
