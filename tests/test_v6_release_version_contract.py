...
def test_current_release_version_contract():
    ...
    assert 'default="8.0.0"' in config_source
    assert 'APP_VERSION: "8.0.0"' in compose
    assert frontend["version"] == "8.0.0"
    assert 'version = "8.0.0"' in sdk
    assert 'appVersion: "8.0.0"' in helm
    assert 'APP_VERSION: "8.0.0"' in ci
