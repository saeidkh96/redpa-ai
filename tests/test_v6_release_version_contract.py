...
def test_current_release_version_contract():
    ...
    assert 'default="10.0.0"' in config_source
    assert 'APP_VERSION: "10.0.0"' in compose
    assert frontend["version"] == "10.0.0"
    assert 'version = "10.0.0"' in sdk
    assert 'appVersion: "10.0.0"' in helm
    assert 'APP_VERSION: "10.0.0"' in ci
