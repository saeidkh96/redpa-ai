...
def test_current_release_version_contract():
    ...
    assert 'default="19.2.0"' in config_source
    assert 'APP_VERSION: "19.2.0"' in compose
    assert frontend["version"] == "19.2.0"
    assert 'version = "19.2.0"' in sdk
    assert 'appVersion: "19.2.0"' in helm
    assert 'APP_VERSION: "19.2.0"' in ci
