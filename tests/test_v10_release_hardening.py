...
def test_v10_machine_readable_versions_are_consistent():
    assert 'default="18.2.0"' in read("backend/app/core/config.py")
    assert 'APP_VERSION: "18.2.0"' in read("docker-compose.yml")
    assert "version='18.2.0'" in read("backend/app/ops_v9/agent_server.py")
    assert "'version':'18.2.0'" in read("backend/app/ops_v9/agent_server.py")
    assert json.loads(read("frontend/package.json"))["version"] == "18.2.0"
    assert 'version = "18.2.0"' in read("sdk/python/pyproject.toml")
    assert 'appVersion: "18.2.0"' in read("deploy/helm/redpa/Chart.yaml")
    assert 'APP_VERSION: "18.2.0"' in read(".github/workflows/ci.yml")


def test_v10_gate_runs_phase3_lifecycle_and_full_regression():
    gate = read(".github/workflows/v10-governance-gate.yml")
    assert 'APP_VERSION: "18.2.0"' in gate
