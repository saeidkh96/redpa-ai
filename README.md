...
The Planner analyzes a request and selects the appropriate execution path.
...
Specialist agents run as separate services and expose A2A-compatible Agent Cards.
...
The durable workflow layer persists state and allows interrupted or failed execution to continue later.
...
RedPA AI implements MCP as a tool platform rather than embedding every integration directly into the backend.
...
Durable workflows are designed for tasks that cannot be treated as one synchronous request.
...
The Agent Memory layer enables agents to reuse relevant information across workflows.
...
Tempo stores distributed traces received through the OpenTelemetry Collector.
...
Reusing the same key with a different request produces a conflict response.
...
The repository currently contains extensive tests covering:
...
The release validation currently passes the complete test suite.
...
Release automation can run on version tags.
