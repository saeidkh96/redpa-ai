...
class DistributedSubtask(BaseModel):
...
class DistributedWorkflowRequest(BaseModel):
    model_config = ConfigDict(
        extra="forbid",
        str_strip_whitespace=True,
    )
    ...
    max_parallelism: int = Field(default=4, ge=1, le=10)
    timeout_seconds: float = Field(default=120.0, ge=1.0, le=900.0)
    approval_granted: bool = False


class DistributedSubtaskResult(BaseModel):
    subtask_id: str
    instruction: str
...
class DistributedWorkflowResponse(BaseModel):
    success: bool
